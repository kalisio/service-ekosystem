...
import { segmentEach as turfSegmentEach } from '@turf/meta'
import * as GeoTIFF from 'geotiff'

const debug = makeDebug('k2:elevation')

function execBg (exe, args, logFile, { cwd = null, env = null, exitIfFails = true } = {}) {
  const opts = {
    cwd,
    stdio: 'inherit',
    env
  }
  const cl = `${exe} ${args.join(' ')} >> ${logFile} 2>&1`
...
async function elevation (geojson, query) {
  ...
  const allSegments = []
  turfSegmentEach(feature, (segment, featureIndex, multiFeatureIndex, geometryIndex, segmentIndex) => {
    ...
    const length = turfLength(segment, { units: 'kilometers' }) * 1000
  ...
  await parallelExec(allTasks, concurrency)
  ...
  for (const task of allTasks) {
    ...
    segments.push(Array.from(data[0], (v, i) => {
      const point = turfAlong(task.segment.segment, (task.segment.sampleOffset + i) * res[0], { units: 'meters' })
  ...
  return turfFeatureCollection([].concat(...segments))
}
