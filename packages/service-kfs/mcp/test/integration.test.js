...
describe('KFS MCP Server — Integration (requires MongoDB)', () => {
  ...
  it('list_collections discovers both feature services', async () => {
  ...
  it('get_features returns all stations without any filter', async () => {
    const data = await getFeatures({ name: 'hubeau-stations' })
    assert.strictEqual(data.numberMatched, nbStations)
  })

  it('get_feature retrieves a single station by id', async () => {
  ...
  it('get_features — bbox filters stations to a small area', async () => {
    const data = await getFeatures({ name: 'hubeau-stations', bbox: '6.39,48.30,6.41,48.32' })
    assert.strictEqual(data.numberMatched, 1)
  })

  it('get_features — datetime instant returns observations at that exact time', async () => {
  ...
  it('get_features — datetime closed interval returns observations in range', async () => {
    const data = await getFeatures({
      name: 'hubeau-observations',
  ...
  it('get_features — property filter on numeric value H = 0.63', async () => {
    const data = await getFeatures({
      name: 'hubeau-observations',
      properties: { H: 0.63 }
    })
  ...
  it('get_features — cql-json eq: H = 0.63 returns one observation', async () => {
    const data = await getFeatures({
      name: 'hubeau-observations',
  ...
  it('get_features — cql-json lt: H < 0.4 excludes the 9 non-standard observations', async () => {
    const data = await getFeatures({
      name: 'hubeau-observations',
  ...
  it('get_features — cql-json gt: H > 0.5 returns the H = 0.63 observation', async () => {
    const data = await getFeatures({
      name: 'hubeau-observations',
  ...
  it('get_features — cql-json and(gte, lte): H in [0.63, 0.63] returns one', async () => {
    const data = await getFeatures({
      name: 'hubeau-observations',
  ...
  it('get_features — cql-json not(lt): NOT H < 0.63 returns H >= 0.63', async () => {
    const data = await getFeatures({
      name: 'hubeau-observations',
  ...
  it('get_features — cql-json isNull: H IS NULL returns 0 (no null values in dataset)', async () => {
    const data = await getFeatures({
      name: 'hubeau-observations',
  ...
  it('get_features — cql-json not(isNull): H IS NOT NULL returns all observations', async () => {
    const data = await getFeatures({
      name: 'hubeau-observations',
  ...
  it('get_features — cql-json like: TypStation = LIMNI', async () => {
    const data = await getFeatures({
  ...
  it('get_features — cql-json like nocase: TypStation = limni (case-insensitive)', async () => {
    const data = await getFeatures({
      name: 'hubeau-stations',
  ...
  it('get_features — cql-json like with % wildcard: LbStationH contains Wasselonne', async () => {
    const data = await getFeatures({
      name: 'hubeau-stations',
  ...
  it('get_features — cql-json and(like, isNull): LIMNI stations with null InfluLocal', async () => {
    const data = await getFeatures({
      name: 'hubeau-stations',
      filter: JSON.stringify({
        op: 'and',
  ...
  it('get_features — cql-json t_during: observations in a time interval', async () => {
    const data = await getFeatures({
      name: 'hubeau-observations',
      filter: JSON.stringify({
        op: 't_during',
  ...
  it('get_features — cql-json s_intersects: stations inside a small polygon', async () => {
    const data = await getFeatures({
      name: 'hubeau-stations',
      filter: JSON.stringify({
        op: 's_intersects',
  ...
  it('get_features — cql-text IS NULL: stations with no InfluLocal', async () => {
    const data = await getFeatures({
      name: 'hubeau-stations',
  ...
  it('get_features — cql-text IS NOT NULL: stations with InfluLocal set', async () => {
    const data = await getFeatures({
      name: 'hubeau-stations',
  ...
  it('get_features — cql-text LIKE: exact match TypStation = LIMNI', async () => {
    const data = await getFeatures({
      name: 'hubeau-stations',
  ...
  it('get_features — cql-text ILIKE: case-insensitive match TypStation = limni', async () => {
    const data = await getFeatures({
      name: 'hubeau-stations',
  ...
  it('get_features — cql-text LIKE with % wildcard: LbStationH contains Wasselonne', async () => {
    const data = await getFeatures({
      name: 'hubeau-stations',
  ...
  it('get_features — cql-text S_INTERSECTS: stations inside WKT polygon', async () => {
    const data = await getFeatures({
      name: 'hubeau-stations',
